...
public void captureScreenShot(ITestResult result, String status) {
	...
	File scrFile = ((TakesScreenshot) Testscript.driver).getScreenshotAs(OutputType.FILE);
...
// To create folder to store screenshots new File(destDir).mkdirs(); 
// Set file name with combination of test class name + date time. 
	String destFile = passfailMethod + " - " + dateFormat.format(new Date()) + ".png"; 

	try { // Store file at destination folder location 
		FileUtils.copyFile(scrFile, new File(destDir + "/" + destFile)); } catch (IOException e) { e.printStackTrace(); 
		}
	} 
